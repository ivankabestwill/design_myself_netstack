...
use ::tools::{EXIT};
use ::flow::{Flow, flow_change_protocol};
...
use ::packet::{Packet,  PACKET_FLAGS_TOSERVER, PACKET_FLAGS_PSEUDO_STREAM_END};
...
pub enum APPLAYER_DATA<'a>{
    ...
    APPLAYER_DATA_NONE, // no more data for now.
    APPLAYER_DATA_EOF,
...
pub fn get_applayer_flag_from_stream(packet: &Packet, ssn: &Rc<RefCell<TcpSession>>,
                                     stream: & Rc<RefCell<Stream>>, this_dir: bool) -> APPLAYER_FLAGS_TYPE{
    ...
    if ssn.borrow().state == TCP_SSN_CLOSED{
...
pub fn stream_applayer_hand_tcp_data(flow: & Arc<RwLock<Flow>>, ssn: &Rc<RefCell<TcpSession>>,
                                     stream: & Rc<RefCell<Stream>>, flags: APPLAYER_FLAGS_TYPE, data: APPLAYER_DATA) -> bool{

    if check_flag!(ssn.borrow().stream_flags, STREAM_FLAGS_APPLAYER_DISABLED){
        debug!("{} with STREAM_FLAGS_APPLAYER_DISABLED is set, return it.", ssn.borrow());
        return true;
    }

    let alproto = if check_flag!(flags, APPLAYER_FLAGS_STREAM_TOSERVER){
        rw_lock_read!(flow, false).alproto_ts.clone()
    }else if check_flag!(flags, APPLAYER_FLAGS_STREAM_TOCLIENT){
        rw_lock_read!(flow, false).alproto_tc.clone()
    }else{
    ...
    if check_flag!(flags, APPLAYER_FLAGS_STREAM_GAP){
        if alproto == ApplayerProto::APPLAYER_UNKNOWN{
            add_flag!(stream.borrow_mut().stream_flags, STREAM_FLAGS_APPPROTO_DETECTION_COMPLETED);
            debug!("APPLAYER_UNKNOWN {} du to GAP in stream start.", rw_lock_read!(flow, false));
        }else{
            do_call_applayer_parser_to_parse(flags, data, rw_lock_read!(flow, false).alproto.clone());
        }
        return true;
    }
    ...
    if alproto == ApplayerProto::APPLAYER_UNKNOWN &&
        ...
        rw_lock_write!(flow, false).alproto_orig = rw_lock_read!(flow, false).alproto.clone();
        debug!("protocol change, old {} ", rw_lock_read!(flow, false).alproto_orig);
        applayer_proto_detect_reset(flow);
        tcp_proto_detect();

        // follow suricata, some event handle, ignore it .
    }else{
        if rw_lock_read!(flow, false).alproto != ApplayerProto::APPLAYER_UNKNOWN{
            do_call_applayer_parser_to_parse(flags, data, rw_lock_read!(flow, false).alproto.clone());
        }
    }
...
fn applayer_proto_detect_reset(flow: & Arc<RwLock<Flow>>){
